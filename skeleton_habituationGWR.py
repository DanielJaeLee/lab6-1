...
for t in range(nTs):
    ...
    if x[t]>0: #Some sort of stimulation
        v *= dec #Stimulation's weight needs to decrease each time
        
    if t>3 and sum(x[t-4:t])==0 and v<stv:
        v += (stv-v)*.05 #Let's forget this association! We learned what will happen if it forgets for a while. 
